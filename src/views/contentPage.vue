  ...
  <div class="container">
    ...
    <div id="information">
      <div class="card border-radius-20px">
        <div class="content">
          <h2 class="title">{{ desti }}</h2>
          <p class="copy">{{ cityIntroduction }}</p>
    ...
    <div id="special-requests">
      <h1 class="font-belanosima title-medium color-apple-wight text-center margin-bottom-20px">Wander Wise</h1>
      <div class="height-50px bg-wight box-center display-table border-radius-25px">
        <input class="width-200px display-table-cell vertical-center text-medium font-belanosima margin-left-20px"
               placeholder="Hi~ Any Special Requests?" type="text" @focus="showTips" v-model="demanding"/>
      ...
      <div v-if="setTips" class="width-300px box-center">
        <div id="tips" v-for="(tip,index) in tips" :key="index" @click="demanding=tip"
...
export default {
  ...
  props:['departure','plan','demo_plan'],
  data() {
    return {
      // 使用 data() 方法返回的数据对象中的 departure 属性
      depat: '',
      desti: ''
    };
  },
  mounted() {
    // 在组件挂载完成后，从本地存储中获取 'departure' 的值，并将其赋值给 departure 属性
    this.desti = localStorage.getItem('destination');
    this.depat = localStorage.getItem('departure');
  },
  ...
  setup(props) {
    ...
    //let information = ref('');
    let cityIntroduction = ref('');
    let additional_info = reactive([]);
    console.log(props.demo_plan)
    // let jsonString = JSON.stringify(props.plan)
    console.log(props.plan)
    //processData(daily_plan_list, information, additional_info, props.demo_plan)
    processData(daily_plan_list, cityIntroduction, additional_info, props.plan)
    ...
    function submit() {
      request({
        ...
        method: 'post',
        params: {
          demand: demanding.value,
          demo_plan: props.plan
        },
        ...
        router.push({
          name: 'result',
          query: {
            departure: this.depat,
            plan: JSON.stringify(resp.data),
            demo_plan: JSON.stringify(resp.data)
          }
        })
        ...
        Swal.fire({
          icon: 'error',
          title: 'Non-compliant Information',
          text: 'Please enter information that meets the format requirements.',
